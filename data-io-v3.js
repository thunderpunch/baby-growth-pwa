...
function workbookFile(payload){
  ...
  const profileRows=[["版本","生效日期","宝宝名","出生日期","性别","饮食阶段","喂养方式","睡眠环境","常用哄睡方式","平日放床","周末放床","主要问题"]];
  payload.profileVersions.forEach(p=>profileRows.push([`V${p.version}`,p.effectiveFrom,p.base?.name||"",p.base?.birthDate||"",p.base?.sex||"",p.stage?.dietStage||"",p.stage?.feedingMode||"",p.stage?.sleepEnvironment||"",p.stage?.settlingMethod||"",p.stage?.weekday?.bedtime||"",p.stage?.weekend?.bedtime||"",p.stage?.mainIssue||""]));
